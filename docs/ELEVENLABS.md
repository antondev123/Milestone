...
**"speaking" is not always followed by "listening".** In `trip-mu81rfl5` the SDK closed two utterances with `listening` and a phantom `speaking` in the same millisecond and then sent nothing: the block had been heard in full, the client waited for a listening that never came, and the queued checkpoint was never asked. Every `speaking` now arms a watchdog (`SPEAKING_WATCHDOG_MS` 1.5 s past the utterance's minimum speaking time): if the mode is still `speaking` but `isSpeaking` is false and the output meter is silent, the client logs `mode_change {mode: "listening", inferred: true}` and runs the listening logic itself. A real `listening` clears it.

**The "say it" nudge waits for tools.** The 3 s unspoken-reply watchdog is a user turn to ElevenLabs, and a user turn abandons a tool in flight ("Tool execution was abandoned due to user input"); the LLM then answers "say it" with the *previous* t. Starting a tool clears the previous reply's watchdog and the nudge never fires while a tool is running. "say it" counts as synthetic for the interruption check, like "continue".
...
**No auto-continue across a learner turn.** The client arms "continue" only from a tool reply with `more: true`, and disarms it the moment a real learner transcript arrives or while a tool is in flight (`thinkingRef`). Before this, the agent's filler while `ask` ran ("Looking into your question now") ended, the 700 ms timer fired "continue", and the answer was spoken over by the next block. The prompt also forbids fillers; the soft-timeout "One moment." covers the wait.

**Cut-off turns teach the LLM to cut itself off.** The platform keeps an interrupted turn in the history as `"<what was said>..."`, and Gemini Flash then imitates it: in trip-mu83gskk every long `t` after the first pause stopped at ~55 words with a "..." and no interruption, and auto-continue (which keys off the transcript's word count) marched on, so ~75% of two blocks were never heard. Two guards. Client: a transcript that ends in "..." with under 80% of the tool's words is logged `truncated`; a block posts `interrupted { spoken, quiet }` so the next `next` serves the rest of the block from the next sentence with no prefix; anything else gets one "say the rest" turn once the words are out. Prompt: say all of `t`, never end with "...", earlier "..." turns were the learner cutting in. **Interruptions resume mid-block.** `onAgentResponseCorrection` gives the exact words said before a barge-in or our "pause" turn; the client posts them with `interrupted` and `next` re-reads from the sentence that was cut ("Back to it. Even when we have gathered…"), not the top of the ~250-word block. `spokenUpTo` in `src/lib/cursor.ts` matches a contiguous run of sentences, so a stray match cannot skip text; no match = whole block, as before.
...
**Latency and the wait filler.** Lookup tools return in ~15 ms. `answer` (open questions, Sonnet 5) is ~3–4 s and `ask` (Sonnet 5, cached, low effort) ~2–4 s. The platform's soft timeout covers the wait: at 2.5 s it says "One moment." (then "Let me check.", at most two per turn), and because a soft-timeout message keeps the LLM turn open the tool's `t` is still spoken when it lands. The clock runs from the end of the learner's turn and includes the agent's own think time (~0.7–1 s), so at 0.8 s it fired on every lookup too and "Nearly there" in front of a question read as a verdict (trip-mu7zpaw4); 2.5 s clears the lookups, so the filler only ever plays for Claude. Keep the phrases neutral: no praise, no progress words. A question asked *while a question is open* costs both (classify in `answer`, then `ask`).
...
- "say the rest" → the unsaid part of the last `t` (the client's truncated-turn guard). Both count as synthetic turns.
- Say all of `t` to its last word; never end with "..." (see §1, cut-off turns).
...
| `end_trip` | – | ends the trip, spoken summary. Only on "I'm done / I've arrived / end the trip"; "stop" is a hold. The client hangs up once the line has been spoken (transcript arrived + words/220 wpm), 20 s fallback | 6 s |
...
8. "I'm done" → the whole spoken summary ("Trip done … Next leg picks up at …"), then the app navigates to the summary page. The session log's closing transcript row must be `interrupted: false`; `status ending by:agent` lands after it, not 4 s after the tool result.
8b. Two screens: with a Listen trip running, open Hands-on on another device and press End session. The Listen tab hangs up within one block and lands on its own summary (its `next` got 409 `stale`). Start a new Listen trip; the Hands-on tab's next leg tap shows "This session was ended on another screen". The new trip's session log has no calls from the other device.
