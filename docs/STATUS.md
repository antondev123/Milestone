...
Updated: 2026-09-18 (end of scaffold session)
...
- Task 4: scripts/ingest.ts (Claude Sonnet, structured JSON output, sanity checks). Sample course raw + a hand-authored lesson.json (4 segments, 9 questions) so the app runs before ingest. **Ingest not yet run live: no ANTHROPIC_API_KEY on this machine.**
...
## In progress / needs a human with keys
...
- Run `npm run ingest` once with a key and compare its lesson.json to the hand-written one; commit whichever reads better.
- Real LLM grading: `/api/grade` open answers currently use keyword fallback without `ANTHROPIC_API_KEY`.
...
- Text mode grading with no key accepts any answer that shares ~30% of long words with the model answer. Fine for a rehearsal, not for judging. Add the key.
