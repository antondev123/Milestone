"use client";
// Phone-only bottom sheet for the assistant (the desktop layout shows it as a column instead).
import { useEffect } from "react";
import { ChatIcon, CloseIcon } from "@/components/carry/Icons";

export function AssistantSheet({ open, onClose, title = "Ask about this passage", children }: { open: boolean; onClose: () => void; title?: string; children: React.ReactNode }) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="fixed inset-0 z-30 lg:hidden">
      ...
      <div role="dialog" aria-label={title} className="absolute inset-x-0 bottom-0 flex h-[62dvh] flex-col rounded-t-[20px] bg-ground px-5 pt-2 pb-4 text-ink">
        <div className="mx-auto mb-2 h-1 w-10 rounded-full bg-track" aria-hidden="true" />
